...
> ⚠️ **2026-08 이후 작업은 `HANDOFF-2026-08.md`를 먼저 읽으세요.** 핑크팽귄 리브랜딩,
> 브랜치 구조(`pink-penguin-mascot`가 현재 최신), 데이터 원장(store_events/store_customers)
> 스키마가 이 문서(CLAUDE.md)에는 반영돼 있지 않습니다. 게임 엔진 등 코드 구조 설명은
> 이 문서가 여전히 정확하니 계속 참고하세요.
